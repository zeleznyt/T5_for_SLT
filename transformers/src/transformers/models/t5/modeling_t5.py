...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False):
        ...
        self.registers_special_position_tokens = config.registers_special_position_tokens
        self.registers_view_homogenous = config.registers_view_homogenous

        if self.has_relative_attention_bias:
            if self.registers_special_position_tokens:
                if self.registers_view_homogenous:
                    self.relative_attention_bias = nn.Embedding(self.relative_attention_num_buckets+self.n_registers+1, self.n_heads)
                    # with torch.no_grad():
                    #     self.relative_attention_bias
                else:
                    self.relative_attention_bias = nn.Embedding(self.relative_attention_num_buckets+self.n_registers*2, self.n_heads)
            else:
    ...
    @staticmethod
    def _relative_position_bucket(relative_position, memory_position, bidirectional=True, num_buckets=32, max_distance=128, registers_special_position_tokens=False, registers_view_homogenous=True, n_registers=0, register_placement=None): #HERE
        """
        ...
        if registers_special_position_tokens:
            if bidirectional:
                max_buckets = num_buckets*2
            else:
            ...
            if bidirectional:
                ...
                #         for j in range(i+1):
                #             relative_buckets[i, j] = max_buckets+ n_registers
                # else:
                #     for i in range(n_registers):
                #         for j in range(i+1):
                #             relative_buckets[i, j] = max_buckets+ n_registers + j
#relative_buckets.cpu().detach().numpy()
    ...
    def compute_bias(self, query_length, key_length, device=None):
        ...
        relative_position_bucket = self._relative_position_bucket(
            ...
            registers_special_position_tokens=self.registers_special_position_tokens,
            registers_view_homogenous=self.registers_view_homogenous,
            n_registers=self.n_registers,
            register_placement=self.register_placement,
        )
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        hidden_states = encoder_outputs[0]
        if self.config.registers_remove_before_decode:
            if self.config.register_placement == 'front':
                hidden_states = hidden_states[:,self.config.n_registers:,:]
                attention_mask = attention_mask[:,self.config.n_registers:]
            elif self.config.register_placement == 'before_padding':
                B, T, D = hidden_states.shape
                device = hidden_states.device

                valid_len = attention_mask.sum(dim=1)  # [B]
                remove_start = valid_len - self.config.n_registers
                positions = torch.arange(T, device=device).unsqueeze(0)  # [1, T]

                keep_mask = ~(
                        (positions >= remove_start.unsqueeze(1)) &
                        (positions < valid_len.unsqueeze(1))
                )

                hidden_states = hidden_states[keep_mask].view(B, -1, D) #hidden_states[0].cpu().detach().numpy()
                attention_mask = attention_mask[keep_mask].view(B, -1)
                # for b in range(attention_mask.size(0)):
                #     valid_len = int(attention_mask[b].sum().item())
                #     hidden_states[b] = torch.cat((hidden_states[b, :valid_len - self.config.n_registers, :], hidden_states[b, valid_len:, :]), dim=0)
                #     attention_mask[b] = torch.cat((attention_mask[b, :valid_len - self.config.n_registers], attention_mask[b, valid_len:]), dim=0)
            elif self.config.register_placement == 'after_padding':
                hidden_states = hidden_states[:, :len(attention_mask[0])-self.config.n_registers, :]
                attention_mask = attention_mask[:, :len(attention_mask[0])-self.config.n_registers]
